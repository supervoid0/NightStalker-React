...
function MusicPreview({MusicData, getMoreMusic}) {
    return (
        <div className="w-full lg:w-6/12 xl:w-7/12 2xl:w-6/12 flex-shrink-0 overflow-y-auto max-h-460 sm:max-h-440 sm:mt-4 sm:border border-gray-800 sm:p-4 rounded" >
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3 gap-2 sm:gap-4 grid-flow-row">
                ...
                {MusicData.next? <ShowMoreCards getMore={getMoreMusic}/>:null}
            </div>
        </div>
    )
}
...
function ShowMoreCards({getMore}){
    
    return(
        <div className="border border-gray-800 hover:border-blue-600 rounded p-2 col-span-1 sm:col-span-2 lg:col-span-1 xl:col-span-2 flex bg-green-500 hover:bg-blue-500 hover:bg-opacity-10 bg-opacity-5 cursor-pointer" onClick={()=>getMore()}>
